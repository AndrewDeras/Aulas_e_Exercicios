...
const divClickMessage = ({target}) => {
  const elementClicked = target.tagName.toLowerCase();
  if (elementClicked === 'div') {
    h2.textContent = `Clicou na div.`;
    return
  }
  h2.textContent = `Clicou no ${elementClicked}, filho da div.`
};

div.addEventListener('click', divClickMessage);
...
const showMouseLocalization = ({ offsetX, offsetY }) => {
  egg.textContent = `Eixo X: ${offsetX} | Eixo Y: ${offsetY}`;
};

egg.addEventListener('mousemove', showMouseLocalization);
...
const isSomePersonFrontendDev = people.some(({ profession }) =>
