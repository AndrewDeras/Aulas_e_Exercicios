...
inputUserName.addEventListener('input', ({ target }) => {
  const inputValue = target.value;
  const validateUserName = usernameRegex.test(inputValue);
  ...
  if (!validateUserName) {
    ...
    target.insertAdjacentElement('afterend', paragraphUserNameFeedback);
    return
  }
...
const insertParagraphIntoDOM = (paragrapgh, text, className, previousSibling) => {
  paragrapgh.textContent = text;
  paragrapgh.setAttribute('class', className);
  previousSibling.insertAdjacentElement('afterend', paragrapgh);
};

form.addEventListener('submit', event => {
  event.preventDefault();

  const validateUserName = usernameRegex.test(inputUserName.value);
